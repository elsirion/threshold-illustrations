"""Generate the illustrative chart; see threshold-diversity-model.md for assumptions."""
...
IMPLEMENTATION_ATTACK_PROBABILITY = (SINGLE_IMPLEMENTATION_FLOOR - SHARED_FAILURE_PROBABILITY) / RESIDUAL_CONDITIONAL_RISK
...
def implementation_risk(effort):
    """Codebase compromise probability conditional on an implementation attack."""
    return RESIDUAL_CONDITIONAL_RISK + (INITIAL_CONDITIONAL_RISK - RESIDUAL_CONDITIONAL_RISK) * exp(-effort)


def threshold_risk(effort, allocation):
    """Independent conditional compromises expose each codebase's whole signer group."""
    p = implementation_risk(effort / len(allocation))
    return sum(
        prod(p if breached else 1 - p for breached in state)
        for state in product((False, True), repeat=len(allocation))
        if sum(size for size, breached in zip(allocation, state) if breached) >= THRESHOLD
    )


def system_risk(effort, allocation):
    # Shared failure and an implementation-specific attack are disjoint scenarios.
    return SHARED_FAILURE_PROBABILITY + IMPLEMENTATION_ATTACK_PROBABILITY * threshold_risk(effort, allocation)


def check_model():
    for effort in (0, 1, 3, 10, 20, 100):
        p1, p2, p4 = (implementation_risk(effort / n) for n in (1, 2, 4))
        expected = (p1, p2**2, 4*p4**3 - 3*p4**4)
        for allocation, probability in zip(STRATEGIES, expected):
            assert sum(allocation) == SIGNERS, 'Every strategy must use ten signers'
            assert isclose(threshold_risk(effort, allocation), probability, abs_tol=1e-12)
            assert SHARED_FAILURE_PROBABILITY <= system_risk(effort, allocation) <= 0.05
    ...
    assert system_risk(20, STRATEGIES[2]) < system_risk(20, STRATEGIES[1]) < system_risk(20, STRATEGIES[0])
...
text(72, 93, 'Threshold Security Illustrative Model', 47, weight='bold')
text(72, 140, 'Independent implementations can make one compromised codebase insufficient to authorize a signature.', 23, '#576875')
text(130, 214, 'Likelihood of unauthorized signing', 25, weight='bold')
...
py = lambda probability: y0 - (y0-y1)*probability/0.05
for tick in range(6):
    percent = tick
    y = py(percent/100)
    line(x0, y, x1, y)
...
for allocation, color, y in zip(STRATEGIES, COLORS, (633, 687, 727)):
...
text(1204, 235, '10 SIGNERS · 7 REQUIRED', 22, '#576875', 'bold')
labels = ('1 implementation', '2 implementations', '4 implementations')
requirements = ('Needs 1 compromised codebase', 'Needs both A and B', 'Needs any 3 compromised codebases')
...
text(101, 893, 'Fewer shared failure modes—not necessarily fewer bugs.', 31, '#075e50', 'bold')
text(72, 961, 'Toy probabilities; equal hardening budget. Common-failure floor: 0.01%. Shared attack exposure also correlates risk.', 20, '#576875')
text(72, 992, 'Shared flaws limit the benefit. Signing safety only; implementation startup costs excluded.', 20, '#576875')
svg.extend(('</g>', '</svg>'))
(OUT/'threshold-diversity.svg').write_text('\n'.join(svg))
with (OUT/'threshold-diversity.csv').open('w', newline='') as handle:
